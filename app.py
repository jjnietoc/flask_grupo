...
def index():
    ...
    post = Post.query.all()
    return render_template('indexCss.html', posts=post) #para pooder ver este indexCss.html
...
@app.route("/insert")
def insert():
    u = User(username = "jjnieto", email = "jjnc@hotmail.com")
    u.set_password("hola")
    db.session.add(u)
    db.session.commit()
    return "Insertado"
...
@app.route('/usuario/new_post', methods=['GET', 'POST'])
@login_required
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        data_post = Post(body=form.body.data, user_id=current_user.id)
